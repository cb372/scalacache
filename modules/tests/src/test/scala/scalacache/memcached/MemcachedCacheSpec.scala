...
class MemcachedCacheSpec
  ...
  if (!memcachedIsRunning) {
    ...
    it should "return the value stored in Memcached" in {
      ...
      whenReady(MemcachedCache[IO](client).get[Int]("key1").unsafeToFuture()) {
        _ should be(Some(123))
      }
    }

    it should "return None if the given key does not exist in the underlying cache" in {
      whenReady(MemcachedCache[IO](client).get[Int]("non-existent-key").unsafeToFuture()) {
        _ should be(None)
      }
    }
    ...
    it should "store the given key-value pair in the underlying cache" in {
      whenReady(MemcachedCache[IO](client).put("key2")(123, None).unsafeToFuture()) { _ =>
    ...
    it should "store the given key-value pair in the underlying cache" in {
      whenReady(MemcachedCache[IO](client).put("key3")(123, Some(3 seconds)).unsafeToFuture()) { _ =>
    ...
    it should "delete the given key and its value from the underlying cache" in {
      ...
      whenReady(MemcachedCache[IO](client).remove("key1").unsafeToFuture()) { _ =>
