...
package object memoization {
  ...
  /** Perform the given operation and memoize its result to a cache before returning it. If the result is already in the
    ...
    */
  inline def memoize[F[_], V](ttl: Option[Duration])(f: => V)(implicit cache: Cache[F, String, V], config: MemoizationConfig, flags: Flags): F[V] =
  ...
  /** Perform the given operation and memoize its result to a cache before returning it. If the result is already in the
    ...
    *   A result, either retrieved from the cache or calculated by executing the function `f`
    */
  inline def memoizeF[F[_], V](ttl: Option[Duration])(f: F[V])(implicit cache: Cache[F, String, V], config: MemoizationConfig, flags: Flags): F[V] =
