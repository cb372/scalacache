...
trait RedisSerialization {
  ...
  private object Legacy {
    ...
    def serialize(value: Any): Array[Byte] = value match {
      case s: String =>
        withObjectOutputStream(MagicNumbers.STRING, _.writeUTF(s))
      case bs: Array[Byte] =>
        withObjectOutputStream(MagicNumbers.BYTE_ARRAY, { oos =>
          oos.writeInt(bs.length)
          oos.write(bs)
        })
      case i: Int => withObjectOutputStream(MagicNumbers.INT, _.writeInt(i))
      case d: Double =>
        withObjectOutputStream(MagicNumbers.DOUBLE, _.writeDouble(d))
      case l: Long => withObjectOutputStream(MagicNumbers.LONG, _.writeLong(l))
      case any =>
        withObjectOutputStream(MagicNumbers.OBJECT, _.writeObject(any))
    }
