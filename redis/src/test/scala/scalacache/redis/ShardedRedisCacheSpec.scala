...
class ShardedRedisCacheSpec extends RedisCacheSpecBase {
  ...
  def assumingMultipleRedisAreRunning(
      f: (ShardedJedisPool, ShardedJedis) => Unit): Unit = {
    Try {
      val shard1 = new JedisShardInfo("localhost", 6379)
      val shard2 = new JedisShardInfo("localhost", 6380)

      val jedisPool =
        new ShardedJedisPool(new JedisPoolConfig(),
                             java.util.Arrays.asList(shard1, shard2))
      ...
      case Failure(_) =>
        alert(
          "Skipping tests because it does not appear that multiple instances of Redis are running on localhost.")
